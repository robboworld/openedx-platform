...
- **[LMS] Шапка (гость):** выровнен стиль с авторизованным пользователем — у «Course catalog» классы `tab-nav-link` и `discover-new-link`, тот же `href` через `marketing_link('COURSES')`, `active`/`aria-current` по пути (`navbar-not-authenticated.html`); убраны лишний нижний `padding` у пунктов с кнопками Register/Sign in и конфликт с частичным сбросом в `extra/_header.scss` (`_home.scss`, `extra/_header.scss`); усилен сброс `border-bottom` и `padding-bottom` у `header.global-header` против базового edx.
- **[LMS] Главная:** у блока `.home > header .title` сброшены `border-top` и `border-image`, чтобы не проявлялась лишняя линия (`partials/lms/theme/_extras.scss`).
